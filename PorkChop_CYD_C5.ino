...
// Touch uses TFT_eSPI's own SPI instance (getSPIinstance()) with manual CS.
// Both display and touch share SPI2 hardware — we coordinate via CS pins:
// TFT CS (GPIO23) HIGH during touch reads, Touch CS (GPIO1) HIGH during display writes.

static bool _touchMisoReady = false;

static uint16_t _touchRead(uint8_t cmd) {
  SPIClass& spi = tft.getSPIinstance();
  if (!_touchMisoReady) {
    // TFT_eSPI was init'd without MISO (not needed for display).
    // Reconfigure SPI2 to add MISO=9 for touch reads without resetting display.
    spi.begin(TOUCH_CLK_PIN, TOUCH_MISO_PIN, TOUCH_MOSI_PIN, -1);
    _touchMisoReady = true;
  }
  spi.beginTransaction(SPISettings(2000000, MSBFIRST, SPI_MODE0));
  digitalWrite(TOUCH_CS_PIN, LOW);
  spi.transfer(cmd);
  uint16_t val = (spi.transfer(0) << 8 | spi.transfer(0)) >> 3;
...
bool getPoint(int& sx, int& sy) {
  ...
  if (millis()-_dbg > 2000) { _dbg=millis();
    Serial.printf("[TOUCH] z1=%d z2=%d p=%d\n", z1, z2, pressure); }

  if (pressure < 400 || pressure > 7500) return false;
  if (z1 == 0 || z1 >= 4095) return false;
  if (z2 >= 4090) return false;

  // Read X/Y — 5 samples, median of middle 3
  uint16_t xs[5], ys[5];
  for (int i = 0; i < 5; i++) {
    xs[i] = _touchRead(0xD1);
    ys[i] = _touchRead(0x91);
  }
  for (int i = 0; i < 4; i++)
    for (int j = i+1; j < 5; j++) {
  ...
  uint16_t rawx = (xs[1]+xs[2]+xs[3])/3;
  uint16_t rawy = (ys[1]+ys[2]+ys[3])/3;
  if (rawx < 100 || rawx > 3950 || rawy < 100 || rawy > 3950) return false;
...
void setup() {
  ...
  // Touch — shares TFT_eSPI's SPI instance, only CS needs init
  pinMode(TOUCH_CS_PIN, OUTPUT); digitalWrite(TOUCH_CS_PIN, HIGH);
