...
bool getPoint(int& sx, int& sy) {
  // Touch disabled temporarily — SPI.begin() in bit-bang was breaking display
  // Will re-enable once display stability confirmed
  return false;
}
...
void setup() {
  ...
  // Touch — bit-bang GPIO init
  // CRITICAL: GPIO6 (CLK) and GPIO7 (MOSI) are owned by TFT_eSPI hardware SPI.
  // We must NOT call pinMode on them — that reconfigures them as GPIO and breaks TFT.
  // Only take ownership of CS and MISO.
  pinMode(TOUCH_CS_PIN,   OUTPUT); digitalWrite(TOUCH_CS_PIN,   HIGH);
  pinMode(TOUCH_MISO_PIN, INPUT);
  // CLK and MOSI will be driven manually during touch reads by temporarily
  // reconfiguring them — but we restore them after each read via the SPI peripheral.
