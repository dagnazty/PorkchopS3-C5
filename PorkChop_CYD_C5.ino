// ============================================================
// M5PORKCHOP → NM-CYD-C5 (ESP32-C5) PORT
// Original: https://github.com/0ct0sec/M5PORKCHOP by 0ct0
// CYD Port: faithfully adapted from full source by xom
// C5 Port: adapted for NM-CYD-C5 hardware
//
// Hardware: NM-CYD-C5 (RockBase-iot / NMTech)
//   ESP32-C5-WROOM-1 RISC-V @240MHz, 16MB Flash, 8MB PSRAM
//   ST7789 2.8" 320x240 TFT   SPI: MOSI=7 MISO=2 CLK=6 CS=23 DC=24
//   XPT2046 touch              SPI: shared bus CLK=6 MISO=2 MOSI=7 CS=1
//   Backlight GPIO 25 HIGH
//   SD card CS=10              GPS: RX=4 TX=5 (Serial2, optional)
//   Shared SPI bus: all three devices (display, touch, SD) use CLK=6 MISO=2 MOSI=7
//
// Arduino IDE Board: ESP32C5 Dev Module
//   Partition: Huge APP (3MB No OTA / 1MB SPIFFS)
//
// Required Libraries:
//   TFT_eSPI by Bodmer (configure with User_Setup.h for C5)
//   XPT2046_Touchscreen by Paul Stoffregen
//   TinyGPSPlus by Mikal Hart
// ============================================================

#include <TFT_eSPI.h>
// XPT2046_Touchscreen not used — touch is bit-bang on NM-CYD-C5 (MISO=GPIO9)
#include <WiFi.h>
...
// ============================================================
// NM-CYD-C5 PIN DEFINITIONS
// ============================================================
#define TFT_BL_PIN    25   // Backlight (active HIGH)
// RGB LED: not present on NM-CYD-C5 — stubbed as no-ops
#define LED_R_PIN     -1
#define LED_G_PIN     -1
#define LED_B_PIN     -1
#define SD_CS_PIN     10   // SD card CS — shared SPI bus
#define SPEAKER_PIN   -1   // No speaker on NM-CYD-C5 — SFX stubbed

// Touch (shared SPI bus with display and SD)
#define TOUCH_CS_PIN   1
#define TOUCH_IRQ_PIN -1   // No IRQ pin on NM-CYD-C5
#define TOUCH_CLK_PIN  6   // shared with TFT and SD
#define TOUCH_MISO_PIN 9   // ACTUAL MISO — GPIO2 is wrong per scan, real pin is GPIO9
#define TOUCH_MOSI_PIN 7   // shared with TFT and SD

// GPS (Serial2) - optional — LP-UART connector P5
#define GPS_RX_PIN     4   // GPIO 4 = NM-CYD-C5 P5 RX
#define GPS_TX_PIN     5   // GPIO 5 = NM-CYD-C5 P5 TX
...
enum class PorkchopMode : uint8_t {
  ...
  SWINE_RADAR,    // Drone RemoteID + AirTag/tracker + Stingray heuristic
  SD_FORMAT,
  CHARGING,
  WEBUI_MODE
};
...
// Touch on shared SPI bus (CLK=6, MISO=9, MOSI=7, CS=1)
// NM-CYD-C5: MISO is GPIO9 (NOT GPIO2 as published pinout states)
// Using bit-bang SPI for touch — hardware SPI peripheral is owned by TFT_eSPI
SPIClass sdSPI(SPI2_HOST);
// SD bus reclaim — re-init SD after display operations reconfigure the shared bus
#define SD_RECLAIM() do { sdSPI.begin(6, 2, 7, SD_CS_PIN); SD.begin(SD_CS_PIN, sdSPI, 4000000); } while(0)

// SD uses sdSPI on shared bus (CLK=6, MISO=2, MOSI=7, CS=10)
// SD uses sdSPI on shared bus (CLK=6, MISO=2, MOSI=7, CS=10)
...
// ============================================================
// SFX — stubbed on NM-CYD-C5 (no speaker hardware)
// ============================================================
namespace SFX {
enum Event : uint8_t {
  NONE=0,DEAUTH,HANDSHAKE,PMKID,NETWORK_NEW,CLIENT_FOUND,SIGNAL_LOST,
  CHANNEL_LOCK,REVEAL_START,ACHIEVEMENT,LEVEL_UP,JACKPOT_XP,ULTRA_STREAK,
  CHALLENGE_COMPLETE,CHALLENGE_SWEEP,CALL_RING,SYNC_COMPLETE,ERROR,CLICK,
  MENU_CLICK,TERMINAL_TICK,BOOT,SIREN,YOU_DIED,UPLOAD_OK,UPLOAD_FAIL
};
void init()            { Serial.println("[SFX] stubbed — no speaker on NM-CYD-C5"); }
void play(Event e)     { (void)e; }
void update()          {}
void stop()            {}
bool isPlaying()       { return false; }
bool isLedcInited()    { return false; }
} // namespace SFX
...
namespace GhostMode {
  ...
  void update();    // call from loop — handles rotation timer
  bool isActive();
...
namespace SwineRadar {
  ...
  uint8_t getDroneCount();
  void    getDroneInfo(uint8_t i, char* id, float* lat, float* lon, float* alt, float* spd);
  uint8_t getTagCount();
  void    getTagInfo(uint8_t i, uint8_t* mac, char* vendor, int8_t* rssi);
  uint8_t getSuspectCount();
  void    getSuspectInfo(uint8_t i, uint8_t* bssid, char* reason, int8_t* rssi);
  uint8_t getSkimmerCount();
  void    getSkimmerInfo(uint8_t i, uint8_t* mac, char* name, int8_t* rssi);
  uint8_t getHostileCount();
  void    getHostileInfo(uint8_t i, uint8_t* mac, char* type, char* detail, int8_t* rssi);
}
namespace SnoutMode {
  void start();
  void stop();
...
void drawTopBar() {
  ...
  switch (currentMode) {
    ...
    case PorkchopMode::SNOUT_MODE:       modeBuf = "SN0UT"; break;
    case PorkchopMode::SWINE_RADAR:      modeBuf = "SW1N3 RADAR"; break;
...
void drawBottomBar() {
  ...
  switch (currentMode) {
    ...
    case PorkchopMode::WARHOG_MODE:
      if (gpsHasFix)
        snprintf(statsBuf, sizeof(statsBuf), "U:%03lu %.1fkm %.0fkm/h",
  ...
  switch (currentMode) {
    ...
    case PorkchopMode::MENU: {
      static const struct { const char* label; const char* hint; } items[] = {
        ...
        {"SNOUT",        "evil twin+deauth+probe"},
        {"SWINE RADAR",  "drone+tag+stingray"},
      ...
      static const int MENU_COUNT = 19;
    ...
    case PorkchopMode::PIGGYBLUES_MODE: {
      auto drawModeOverlay = [&](auto& dst, int yOff) {
        ...
        if (currentMode == PorkchopMode::OINK_MODE) {
          ...
          // Speed + distance line
          if (gpsHasFix) {
            snprintf(buf, sizeof(buf), "%.1f km/h  %.2f km",
              gpsSpeedKmh, WarhogMode::getDistanceKm());
          } else {
          ...
          // Coords + peak speed
          if (gpsHasFix) {
            snprintf(buf, sizeof(buf), "[%.4f,%.4f] pk:%.1f",
              gpsLat, gpsLon, WarhogMode::getPeakSpeedKmh());
          } else {
            snprintf(buf, sizeof(buf), "NETS: %lu", WarhogMode::getTotalNets());
          }
          dst.drawString(buf, 2, yOff+MAIN_H-12);
    ...
    case PorkchopMode::SETTINGS: {
      auto drawSet = [&](auto& dst, int yOff) {
        ...
        // Ghost mode status
        char gb[48];
        snprintf(gb, sizeof(gb), "GHOST: %s  ROT:%umin",
          cfg.ghostMode ? "ON" : "OFF", cfg.ghostInterval);
        dst.drawString(gb, 6, y); y+=dy;
        if (cfg.ghostMode) {
          uint8_t mac[6]; GhostMode::getMAC(mac);
          char ml[32]; snprintf(ml,sizeof(ml),"  %02X:%02X:%02X:%02X:%02X:%02X",
    ...
    case PorkchopMode::SNOUT_MODE: {
      auto drawSnout = [&](auto& dst, int yOff) {
        static const char* tabs[] = {"EVIL TWIN","D3AUTH","H1DD3N"};
        uint8_t tab = SnoutMode::getTab();
        dst.setTextColor(colorFG(), colorBG());
        dst.setTextSize(1); dst.setTextDatum(TC_DATUM);
        // Tab bar
        int tw = DISPLAY_W/3;
        for (uint8_t t=0; t<3; t++) {
          if (t==tab) dst.fillRect(t*tw, yOff, tw, 13, colorFG());
          uint16_t tc = (t==tab) ? colorBG() : colorFG();
          dst.setTextColor(tc, t==tab?colorFG():colorBG());
          dst.setTextDatum(TC_DATUM);
          dst.drawString(tabs[t], t*tw+tw/2, yOff+3);
        }
        dst.setTextColor(colorFG(), colorBG());
        dst.setTextDatum(TL_DATUM);
        int y = yOff+16; const int dy=11;

        if (tab==0) {
          // Evil twins
          uint8_t tc = SnoutMode::getTwinCount();
          if (tc==0) {
            dst.setTextDatum(TC_DATUM);
            dst.drawString("no evil twins detected", DISPLAY_W/2, y+6); y+=dy;
            dst.drawString("sniffing...", DISPLAY_W/2, y+6);
          } else {
            for (uint8_t i=0; i<tc && i<4; i++) {
              char ssid[33]={0}; uint8_t b1[6], b2[6], a1,a2;
              SnoutMode::getTwin(i,ssid,b1,b2,&a1,&a2);
              char line[40];
              snprintf(line,sizeof(line),"!! %.14s",ssid);
              dst.drawString(line, 4, y); y+=dy;
              snprintf(line,sizeof(line)," %02X%02X%02X vs %02X%02X%02X",b1[3],b1[4],b1[5],b2[3],b2[4],b2[5]);
              dst.drawString(line, 4, y); y+=dy;
            }
          }
        } else if (tab==1) {
          // Deauth storm
          bool storm = SnoutMode::isStormActive();
          uint16_t peak = SnoutMode::getDeauthPeak();
          uint16_t rate = SnoutMode::getDeauthRate();
          uint8_t src[6]; SnoutMode::getStormSrc(src);
          char line[48];
          snprintf(line,sizeof(line),"RATE: %u/sec", rate);
          dst.setTextDatum(TC_DATUM);
          dst.drawString(line, DISPLAY_W/2, y); y+=dy;
          snprintf(line,sizeof(line),"PEAK: %u/sec", peak);
          dst.drawString(line, DISPLAY_W/2, y); y+=dy+2;
          if (storm) {
            dst.drawString("!! DEAUTH STORM !!", DISPLAY_W/2, y); y+=dy;
            snprintf(line,sizeof(line),"SRC: %02X:%02X:%02X:%02X:%02X:%02X",src[0],src[1],src[2],src[3],src[4],src[5]);
            dst.setTextSize(1); dst.drawString(line, DISPLAY_W/2, y);
          } else if (peak == 0) {
            dst.drawString("watching for storms...", DISPLAY_W/2, y);
          } else {
            dst.drawString("storm subsided", DISPLAY_W/2, y);
          }
        } else {
          // Hidden SSIDs
          uint8_t hc = SnoutMode::getHiddenCount();
          if (hc==0) {
            dst.setTextDatum(TC_DATUM);
            dst.drawString("no hidden nets found", DISPLAY_W/2, y+6); y+=dy;
            dst.drawString("probing...", DISPLAY_W/2, y+6);
          } else {
            for (uint8_t i=0; i<hc && i<5; i++) {
              uint8_t bssid[6]; char ssid[33]={0}; bool rev=false;
              SnoutMode::getHiddenEntry(i,bssid,ssid,&rev);
              char line[40];
              if (rev) snprintf(line,sizeof(line),">> %.18s",ssid);
              else     snprintf(line,sizeof(line),"?? %02X:%02X:%02X:%02X:%02X:%02X",bssid[0],bssid[1],bssid[2],bssid[3],bssid[4],bssid[5]);
              dst.drawString(line, 4, y); y+=dy;
            }
          }
        }
        dst.setTextColor(colorFG(), colorBG());
        dst.setTextDatum(TC_DATUM);
        dst.drawString("TAP=tab  HOLD=exit", DISPLAY_W/2, yOff+MAIN_H-10);
      };
      if (useSprite) { drawSnout(mainSprite, 0); }
      else           { drawSnout(tft, TOP_BAR_H); }
      break;
    }

    // ── SWINE RADAR ───────────────────────────────────────────────────────────
    case PorkchopMode::SWINE_RADAR: {
      auto drawRadar = [&](auto& dst, int yOff) {
        static const char* tabs[] = {"DR0NE","TAGS","SUSPECT","SK1MMR","H0STILE"};
        uint8_t tab = SwineRadar::getTab();
        dst.setTextColor(colorFG(), colorBG());
        dst.setTextSize(1);
        int tw = DISPLAY_W/5;
        for (uint8_t t=0; t<5; t++) {
          if (t==tab) dst.fillRect(t*tw, yOff, tw, 13, colorFG());
          dst.setTextColor(t==tab?colorBG():colorFG(), t==tab?colorFG():colorBG());
          dst.setTextDatum(TC_DATUM);
          dst.drawString(tabs[t], t*tw+tw/2, yOff+3);
        }
        dst.setTextColor(colorFG(), colorBG());
        ...
        if (tab==0) {
          uint8_t dc = SwineRadar::getDroneCount();
          if (dc==0) {
            dst.setTextDatum(TC_DATUM);
            dst.drawString("skies are clear", DISPLAY_W/2, y+6); y+=dy;
            dst.drawString("watching for RemoteID...", DISPLAY_W/2, y+6);
          } else {
            for (uint8_t i=0; i<dc && i<4; i++) {
              char id[21]={0}; float la,lo,al,sp;
              SwineRadar::getDroneInfo(i,id,&la,&lo,&al,&sp);
          ...
          uint8_t tc = SwineRadar::getTagCount();
          if (tc==0) {
            dst.setTextDatum(TC_DATUM);
            dst.drawString("no trackers detected", DISPLAY_W/2, y+6); y+=dy;
            dst.drawString("AirTag/Tile/SmartTag...", DISPLAY_W/2, y+6);
          } else {
            for (uint8_t i=0; i<tc && i<5; i++) {
              uint8_t mac[6]; char vnd[16]={0}; int8_t rssi;
              SwineRadar::getTagInfo(i,mac,vnd,&rssi);
              char line[40];
          ...
          // Stingray suspects
          uint8_t sc = SwineRadar::getSuspectCount();
          if (sc==0) {
            dst.setTextDatum(TC_DATUM);
            dst.drawString("no suspect APs", DISPLAY_W/2, y+6); y+=dy;
            dst.drawString("watching for IMSI bait...", DISPLAY_W/2, y+6);
          } else {
            for (uint8_t i=0; i<sc && i<5; i++) {
              uint8_t bssid[6]; char reason[32]={0}; int8_t rssi;
              SwineRadar::getSuspectInfo(i,bssid,reason,&rssi);
              char line[40];
        ...
        if (tab == 3) {
          // Card skimmers
          uint8_t kc = SwineRadar::getSkimmerCount();
          if (kc==0) {
            dst.setTextDatum(TC_DATUM);
            dst.drawString("no skimmers detected", DISPLAY_W/2, y+6); y+=dy;
            dst.drawString("HC-05/06/08 FREE2MOVE...", DISPLAY_W/2, y+6);
          } else {
            for (uint8_t i=0; i<kc && i<5; i++) {
              uint8_t mac[6]; char name[24]={0}; int8_t rssi;
              SwineRadar::getSkimmerInfo(i,mac,name,&rssi);
              char line[40];
        ...
        if (tab == 4) {
          // H0STILE — Pwnagotchi + Flipper Zero
          uint8_t hc = SwineRadar::getHostileCount();
          if (hc==0) {
            dst.setTextDatum(TC_DATUM);
            dst.drawString("no hostiles in range", DISPLAY_W/2, y+6); y+=dy;
            dst.drawString("pwna/flipper/pineapple...", DISPLAY_W/2, y+6);
          } else {
            for (uint8_t i=0; i<hc && i<4; i++) {
              uint8_t mac[6]; char type[16]={0}; char detail[48]={0}; int8_t rssi;
              SwineRadar::getHostileInfo(i,mac,type,detail,&rssi);
              char line[48];
      ...
      if (useSprite) { drawRadar(mainSprite, 0); }
      else           { drawRadar(tft, TOP_BAR_H); }
      break;
    }
...
// Touch calibration for NM-CYD-C5 with ST7789 in landscape
#define T_XMIN 200
#define T_XMAX 3900
#define T_YMIN 200
#define T_YMAX 3900

// Bit-bang XPT2046 — hardware SPI is owned by TFT_eSPI, so we bit-bang touch
// MISO=GPIO9 (confirmed by pin scan), CLK=6, MOSI=7, CS=1
static uint16_t _xpt_cmd_raw(uint8_t cmd) {
  // Pure bit-bang — CLK/MOSI must already be configured as GPIO OUTPUT
  uint16_t val = 0;
  ...
  digitalWrite(TOUCH_CLK_PIN, LOW); delayMicroseconds(3);
  digitalWrite(TOUCH_CLK_PIN, HIGH); delayMicroseconds(3);
  for (int i = 11; i >= 0; i--) {
    digitalWrite(TOUCH_CLK_PIN, LOW);
    delayMicroseconds(3);
    digitalWrite(TOUCH_CLK_PIN, HIGH);
    if (digitalRead(TOUCH_MISO_PIN)) val |= (1 << i);
    delayMicroseconds(3);
  }
  digitalWrite(TOUCH_CLK_PIN, LOW);
...
// Touch SPI — dedicated SPIClass on SPI2_HOST so it never conflicts with TFT_eSPI
// TFT_eSPI uses the same SPI2_HOST but we co-exist by manually managing CS:
// TFT CS (GPIO23) stays HIGH during touch reads, Touch CS (GPIO1) stays HIGH otherwise.
// We re-init our SPIClass each read only if needed; begin() is safe because we
// never call SPI.begin() (the global one) — only _touchSPI.begin().
static SPIClass _touchSPI(SPI2_HOST);
static bool _touchSpiReady = false;

static void _touchSpiInit() {
  if (_touchSpiReady) return;
  _touchSPI.begin(TOUCH_CLK_PIN, TOUCH_MISO_PIN, TOUCH_MOSI_PIN, -1);
  pinMode(TOUCH_CS_PIN, OUTPUT); digitalWrite(TOUCH_CS_PIN, HIGH);
  _touchSpiReady = true;
}

static uint16_t _touchRead(uint8_t cmd) {
  _touchSPI.beginTransaction(SPISettings(2000000, MSBFIRST, SPI_MODE0));
  digitalWrite(TOUCH_CS_PIN, LOW);
  _touchSPI.transfer(cmd);
  uint16_t val = _touchSPI.transfer16(0) >> 3;
  digitalWrite(TOUCH_CS_PIN, HIGH);
...
bool getPoint(int& sx, int& sy) {
  ...
  // Read Z pressure to detect touch
  uint16_t z1 = _touchRead(0xB1);
  uint16_t z2 = _touchRead(0xC1);
  int pressure = (int)z1 - (int)z2 + 4095;
  if (pressure < 200 || pressure > 7000) return false;
  if (z1 == 4095 || z1 == 0) return false;

  // Read X/Y — average 3 samples
  uint32_t xsum = 0, ysum = 0;
  for (int i = 0; i < 3; i++) {
    xsum += _touchRead(0xD1);  // X
    ysum += _touchRead(0x91);  // Y
  }
  uint16_t rawx = xsum / 3;
  uint16_t rawy = ysum / 3;
  if (rawx == 0 || rawx == 4095) return false;

  sx = constrain(map(rawx, T_XMIN, T_XMAX, 0, DISPLAY_W-1), 0, DISPLAY_W-1);
  sy = constrain(map(rawy, T_YMIN, T_YMAX, 0, DISPLAY_H-1), 0, DISPLAY_H-1);
  return true;
}
...
static void _sendBurst(const uint8_t* bssid, const uint8_t* sta, uint8_t n) {
  static uint32_t _failCount = 0, _okCount = 0, _lastLog = 0;
  for (uint8_t i=0; i<n; i++) {
    bool ok1 = WSLBypasser::sendDeauthFrame(bssid, NetworkRecon::getChannel(), sta, 7);
    bool ok2 = WSLBypasser::sendDisassocFrame(bssid, NetworkRecon::getChannel(), sta, 8);
    if (ok1 && ok2) _okCount++; else _failCount++;
  }
  if (millis()-_lastLog > 3000) {
    _lastLog = millis();
    Serial.printf("[OINK] deauth tx: ok=%lu fail=%lu ch=%d\n", _okCount, _failCount, NetworkRecon::getChannel());
  }
}
...
void stop() {
  ...
  // Force free vector memory back to heap
  { std::vector<CapturedHandshake> tmp; tmp.swap(_handshakes); }
  { std::vector<CapturedPMKID>     tmp; tmp.swap(_pmkids); }
}
...
// Distance tracking
static float _distanceKm    = 0.0f;
static float _prevLat       = 0.0f;
static float _prevLon       = 0.0f;
static bool  _prevValid     = false;
static float _peakSpeedKmh  = 0.0f;
...
static float _haversine(float lat1, float lon1, float lat2, float lon2) {
  ...
  float a = sinf(dLat/2)*sinf(dLat/2) +
            cosf(lat1*DEG_TO_RAD)*cosf(lat2*DEG_TO_RAD)*sinf(dLon/2)*sinf(dLon/2);
...
void start() {
  ...
  _distanceKm=0.0f; _prevValid=false; _peakSpeedKmh=0.0f;
...
void stop() {
  ...
  NetworkRecon::start();
  char buf[48];
  snprintf(buf,sizeof(buf),"[WARHOG] dist=%.2fkm peak=%.1fkm/h nets=%lu",
    _distanceKm, _peakSpeedKmh, _totalNets);
  Serial.println(buf);
  WarTales::logEvent(buf);
}
...
void update() {
  ...
  if (gpsHasFix && gpsLat != 0.0f && gpsLon != 0.0f) {
    if (_prevValid) {
      float d = _haversine(_prevLat, _prevLon, gpsLat, gpsLon);
      // Only count if moved at least 5m (filter GPS jitter)
      if (d >= 0.005f) {
        _distanceKm += d;
        _prevLat = gpsLat; _prevLon = gpsLon;
      }
    } else {
      _prevLat = gpsLat; _prevLon = gpsLon; _prevValid = true;
    }
    if (gpsSpeedKmh > _peakSpeedKmh) _peakSpeedKmh = gpsSpeedKmh;
  }
...
uint32_t getSavedCount()  { return 0; }
float getDistanceKm()     { return _distanceKm; }
float getPeakSpeedKmh()   { return _peakSpeedKmh; }
...
// ============================================================
// SNOUT MODE — Evil Twin + Deauth Storm + Hidden SSID Prober
// The pig sniffs out the shady stuff.
// ============================================================
namespace SnoutMode {
...
struct EvilTwin {
  char    ssid[33];
  uint8_t bssid1[6], bssid2[6];
  uint8_t auth1, auth2;
  int8_t  rssi1, rssi2;
  uint32_t seenAt;
};
static EvilTwin _twins[8];
...
struct HiddenResult {
  uint8_t bssid[6];
  char    ssid[33];   // revealed SSID (empty if still unknown)
  uint8_t channel;
  bool    revealed;
};
static HiddenResult _hidden[8];
static uint8_t      _hiddenCount = 0;
static uint32_t     _lastProbe   = 0;
static uint8_t      _probeIdx    = 0;

static bool _running    = false;
static uint8_t _tab     = 0;   // 0=twins 1=storm 2=hidden

// ── Deauth frame ISR callback ────────────────────────────────
static void IRAM_ATTR _mgmtCallback(wifi_promiscuous_pkt_t* pkt, wifi_promiscuous_pkt_type_t type) {
  if (type != WIFI_PKT_MGMT) return;
  uint16_t len = pkt->rx_ctrl.sig_len; if (len>4) len-=4;
  if (len < 24) return;
  const uint8_t* p = pkt->payload;
  uint8_t subtype = (p[0] >> 4) & 0x0F;
  // 0xC = deauth, 0xA = disassoc
  if (subtype == 0x0C || subtype == 0x0A) {
    _deauthsThisSec++;
    memcpy(_stormSrcMac, p+10, 6);  // TA = transmitter address
  }
}
...
static void _scanEvilTwins() {
  ...
  int n = (int)nets.size();
  for (int i=0; i<n; i++) {
    if (!nets[i].ssid[0]) continue;  // skip hidden
    for (int j=i+1; j<n; j++) {
      if (!nets[j].ssid[0]) continue;
      if (strcmp(nets[i].ssid, nets[j].ssid) != 0) continue;
      // Same SSID — check if already recorded
      bool known = false;
      for (uint8_t k=0; k<_twinCount; k++)
        if (strcmp(_twins[k].ssid, nets[i].ssid)==0) { known=true; break; }
      if (known || _twinCount >= 8) continue;
      EvilTwin& t = _twins[_twinCount++];
      strncpy(t.ssid, nets[i].ssid, 32); t.ssid[32]=0;
      memcpy(t.bssid1, nets[i].bssid, 6);
      memcpy(t.bssid2, nets[j].bssid, 6);
      t.auth1 = (uint8_t)nets[i].authmode;
      t.auth2 = (uint8_t)nets[j].authmode;
      t.rssi1 = nets[i].rssi;
      t.rssi2 = nets[j].rssi;
      t.seenAt = millis();
...
static void _sendProbe(const uint8_t* bssid, uint8_t channel, const uint8_t* src) {
  uint8_t frame[64]; uint8_t off=0;
  frame[off++]=0x40; frame[off++]=0x00;  // FC: probe request
  frame[off++]=0x00; frame[off++]=0x00;  // duration
  memcpy(frame+off, bssid, 6); off+=6;   // DA = target BSSID
  memcpy(frame+off, src,   6); off+=6;   // SA = our MAC
  memcpy(frame+off, bssid, 6); off+=6;   // BSSID
  frame[off++]=0x00; frame[off++]=0x00;  // seq
  // SSID IE — empty (directed probe)
  frame[off++]=0x00; frame[off++]=0x00;
  // Supported rates IE
  frame[off++]=0x01; frame[off++]=0x08;
...
// ── Harvest probe response SSIDs ─────────────────────────────
// Called from beacon drain — if a hidden net sends a probe response,
// its SSID will appear in subsequent beacons via NetworkRecon
static void _checkRevealedHidden() {
  ...
  for (uint8_t i=0; i<_hiddenCount; i++) {
    ...
    for (const auto& n : nets) {
      if (memcmp(n.bssid, _hidden[i].bssid, 6)==0 && n.ssid[0]) {
        strncpy(_hidden[i].ssid, n.ssid, 32);
        _hidden[i].revealed = true;
        Serial.printf("[SNOUT] HIDDEN REVEALED: '%s' [%02X:%02X:%02X:%02X:%02X:%02X]\n",
...
// ── Populate hidden list from NetworkRecon ───────────────────
static void _refreshHiddenList() {
  ...
  for (const auto& n : nets) {
    ...
    bool known = false;
    for (uint8_t i=0; i<_hiddenCount; i++)
      if (memcmp(_hidden[i].bssid, n.bssid, 6)==0) { known=true; break; }
    if (!known && _hiddenCount < 8) {
      HiddenResult& h = _hidden[_hiddenCount++];
...
void update() {
  ...
  // Deauth storm window (1 second buckets)
  if (now - _deauthWindow >= 1000) {
    if (_deauthsThisSec > _deauthPeak) _deauthPeak = _deauthsThisSec;
    if (_deauthsThisSec >= 10 && !_stormActive) {
      _stormActive = true;
      _stormStartMs = now;
      SFX::play(SFX::DEAUTH);
      Mood::setStatusMessage("DEAUTH STORM!");
      Serial.printf("[SNOUT] DEAUTH STORM: %u/sec from %02X:%02X:%02X:%02X:%02X:%02X\n",
        _deauthsThisSec,
        _stormSrcMac[0],_stormSrcMac[1],_stormSrcMac[2],
        _stormSrcMac[3],_stormSrcMac[4],_stormSrcMac[5]);
    } else if (_deauthsThisSec < 3) {
      _stormActive = false;
    }
    _deauthsThisSec = 0;
    _deauthWindow = now;
  ...
  static uint32_t _lastTwinScan = 0;
  if (now - _lastTwinScan > 3000) { _scanEvilTwins(); _lastTwinScan = now; }

  // Hidden SSID refresh + probe every 4s
  static uint32_t _lastHiddenRefresh = 0;
  if (now - _lastHiddenRefresh > 4000) {
    _refreshHiddenList();
    _checkRevealedHidden();
    _lastHiddenRefresh = now;
  }

  // Send one directed probe every 800ms
  if (_hiddenCount > 0 && now - _lastProbe > 800) {
    uint8_t src[6]; esp_wifi_get_mac(WIFI_IF_STA, src);
    HiddenResult& h = _hidden[_probeIdx % _hiddenCount];
    if (!h.revealed) _sendProbe(h.bssid, h.channel, src);
    _probeIdx++;
    _lastProbe = now;
  }
}

// Accessors for Display::update()
bool isRunning()         { return _running; }
uint8_t getTab()         { return _tab; }
void    setTab(uint8_t t){ _tab = t % 3; }
uint8_t getTwinCount()   { return _twinCount; }
uint8_t getHiddenCount() { return _hiddenCount; }
bool    isStormActive()  { return _stormActive; }
uint16_t getDeauthPeak() { return _deauthPeak; }
uint16_t getDeauthRate() { return _deauthsThisSec; }
void getStormSrc(uint8_t* out) { memcpy(out, _stormSrcMac, 6); }

void getTwin(uint8_t i, char* ssid, uint8_t* b1, uint8_t* b2, uint8_t* a1, uint8_t* a2) {
  if (i>=_twinCount) return;
  strncpy(ssid, _twins[i].ssid, 32);
  memcpy(b1, _twins[i].bssid1, 6); memcpy(b2, _twins[i].bssid2, 6);
  *a1=_twins[i].auth1; *a2=_twins[i].auth2;
}
void getHiddenEntry(uint8_t i, uint8_t* bssid, char* ssid, bool* revealed) {
  if (i>=_hiddenCount) return;
  memcpy(bssid, _hidden[i].bssid, 6);
  strncpy(ssid, _hidden[i].ssid, 32);
...
// ============================================================
// ============================================================
// GHOST MODE — MAC address randomization
// Rotates STA MAC every N minutes while active.
// Locally administered bit set, multicast bit clear.
// ============================================================
namespace GhostMode {

static uint8_t _currentMAC[6] = {};
static uint8_t _realMAC[6]    = {};
static uint32_t _lastRotate   = 0;
static bool     _realSaved    = false;

static void _randomMAC(uint8_t* mac) {
  uint32_t r1 = esp_random();
  uint32_t r2 = esp_random();
  mac[0] = ((uint8_t)(r1 >> 0) & 0xFE) | 0x02; // locally administered, unicast
  mac[1] = (uint8_t)(r1 >>  8);
  mac[2] = (uint8_t)(r1 >> 16);
  mac[3] = (uint8_t)(r2 >>  0);
  mac[4] = (uint8_t)(r2 >>  8);
...
static void _setMAC(const uint8_t* mac) {
  // esp_wifi_set_mac only affects transmitted frames
  // No need to stop NetworkRecon — promiscuous receive is unaffected
...
void apply() {
  if (!_realSaved) {
    esp_wifi_get_mac(WIFI_IF_STA, _realMAC);
    _realSaved = true;
  }
  if (cfg.ghostMode) {
    uint8_t mac[6]; _randomMAC(mac); _setMAC(mac);
    _lastRotate = millis();  // start rotation timer from now
    Display::showToast("GHOST MAC SET", 1500);
    WarTales::logEvent("GHOST MAC rotated");
  } else {
    _setMAC(_realMAC);
    _lastRotate = 0;  // reset so next enable starts fresh
    Display::showToast("MAC RESTORED", 1500);
    WarTales::logEvent("GHOST MODE off - MAC restored");
  }
}

void update() {
  if (!cfg.ghostMode) return;
  // Initialize on first call
  if (_lastRotate == 0) { _lastRotate = millis(); return; }
  uint32_t interval = (uint32_t)cfg.ghostInterval * 60000UL;
  if (millis() - _lastRotate < interval) return;
  // Time to rotate
  uint8_t mac[6]; _randomMAC(mac); _setMAC(mac);
  _lastRotate = millis();
  char buf[48]; snprintf(buf,sizeof(buf),"GHOST auto-rotated (%umin)", cfg.ghostInterval);
  WarTales::logEvent(buf);
}

bool isActive()           { return cfg.ghostMode; }
void getMAC(uint8_t* out) { memcpy(out, _currentMAC, 6); }

} // namespace GhostMode


// WAR TALES — Session diary. The pig's memoirs.
// Logs events to SD:/logs/tale_XXXXXX.txt each session.
// ============================================================
namespace WarTales {
...
static void _writeLine(const char* line) {
  if (!_open || !_logFile) return;
  uint32_t upSec = (millis() - _sessionStart) / 1000;
  char buf[96];
  int n = snprintf(buf, sizeof(buf)-2, "[%3u:%02u] %s\n", upSec/60, upSec%60, line);
  if (n > 0 && n < (int)sizeof(buf)) {
    _logFile.write((const uint8_t*)buf, n);  // no String allocation
  }
  _eventCount++;
  if (_eventCount % 10 == 0) _logFile.flush();
}

void sessionStart() {
  if (!sdAvailable) { Serial.println("[TALES] SD not available"); return; }
  SD_RECLAIM();
  _sessionStart = millis();
  _eventCount = 0;
  char fname[48];
  uint32_t up = millis()/1000;
  // Try /logs/ first, fall back to root if directory missing
  snprintf(fname, sizeof(fname), "/logs/tale_%06lu.txt", up);
  _logFile = SD.open(fname, FILE_WRITE);
  if (!_logFile) {
    // /logs doesn't exist — try root
    snprintf(fname, sizeof(fname), "/tale_%06lu.txt", up);
    _logFile = SD.open(fname, FILE_WRITE);
  }
  if (!_logFile) { Serial.println("[TALES] log open failed — create /logs on SD card"); return; }
  _open = true;
  const char* hdr1 = "=== PORKCHOP CYD — WAR TALE ===\n";
  _logFile.write((const uint8_t*)hdr1, strlen(hdr1));
  char hdr[64];
  snprintf(hdr, sizeof(hdr), "Session start | LVL:%d XP:%lu", XP::getLevel(), XP::getTotalXP());
  _writeLine(hdr);
  Serial.printf("[TALES] %s\n", fname);
}

void sessionEnd() {
  if (!_open) return;
  char footer[64];
  snprintf(footer, sizeof(footer), "Session end | nets:%u deauths:%lu events:%u",
    NetworkRecon::getNetworkCount(), deauthCount, _eventCount);
  _writeLine(footer);
  const char* footer_sep = "=================================\n";
  _logFile.write((const uint8_t*)footer_sep, strlen(footer_sep));
  _logFile.close();
  _open = false;
  Serial.printf("[TALES] session closed (%u events)\n", _eventCount);
}
...
void logCapture(const char* type, const char* ssid) {
  char buf[64]; snprintf(buf, sizeof(buf), "CAPTURE %s: %.32s", type, ssid);
  _writeLine(buf);
  if (_open && _logFile) _logFile.flush();  // flush immediately on capture
}

void logDetection(const char* type, const char* detail) {
  char buf[72]; snprintf(buf, sizeof(buf), "DETECT %s: %.40s", type, detail);
  _writeLine(buf);
  if (_open && _logFile) _logFile.flush();  // flush immediately on detection
}
...
// ============================================================
// SWINE RADAR — Drone RemoteID + AirTag/tracker + Stingray
// Three-tab passive RF threat monitor.
// ============================================================
namespace SwineRadar {

static bool    _running = false;
static uint8_t _tab     = 0;  // 0=drones 1=tags 2=stingray 3=skimmer 4=hostiles

// ── Pwnagotchi / Flipper Zero detection ──────────────────────
struct HostileContact {
  uint8_t mac[6];
  char    type[16];   // "PWNAGOTCHI", "FLIPPER", "PINEAPPLE"
  char    detail[48]; // name/SSID or JSON snippet
  int8_t  rssi;
  uint32_t firstSeen, lastSeen;
  uint16_t seenCount;
};
static HostileContact _hostiles[8];
static uint8_t        _hostileCount = 0;

// Pwnagotchi: always uses BSSID DE:AD:BE:EF:DE:AD
static const uint8_t PWNA_MAC[6] = {0xDE,0xAD,0xBE,0xEF,0xDE,0xAD};

// Flipper Zero WiFi devboard: ESP32-S2-WROVER OUIs
static const uint8_t FLIPPER_OUIS[][3] = {
  {0x24,0x6F,0x28},  // Espressif (official Flipper devboard)
  {0xA4,0xCF,0x12},  // Espressif variant
  {0x24,0xDC,0xC3},  // Espressif variant
  {0x08,0x3A,0xF2},  // Espressif variant
  {0x30,0xAE,0xA4},  // Espressif variant
};
static const uint8_t FLIPPER_OUI_COUNT = 5;

// Flipper Marauder / BlackMagic SSID patterns
static const char* FLIPPER_SSID_PATTERNS[] = {
  "BlackMagic",   // Flipper devboard default debug AP
  "marauder",     // Marauder AP mode
  "marlins",      // Marauder default beacon SSID
  "flipper",      // generic
  "pwned",        // common Marauder beacon list entry
};
static const uint8_t FLIPPER_SSID_COUNT = 5;

// ── Card Skimmer detection ───────────────────────────────────
// HC-05/06/08 modules broadcast WiFi probe requests with known SSID names
// OUI: 00:1A:7D (Shenzhen HC Info Tech), also FREE2MOVE and generic HC modules
struct SkimmerContact {
  uint8_t mac[6];
  char    name[24];   // "HC-05", "HC-06", "HC-08", "FREE2MOVE" etc
  int8_t  rssi;
  uint32_t firstSeen, lastSeen;
  uint16_t seenCount;
};
static SkimmerContact _skimmers[6];
static uint8_t        _skimmerCount = 0;

// Known skimmer module OUI prefixes
static const uint8_t SKIMMER_OUIS[][3] = {
  {0x00,0x1A,0x7D},  // Shenzhen HC Information Technology (HC-05/06/08)
  {0x20,0x16,0x03},  // HC-06 variant
  {0xE0,0xD0,0x83},  // FREE2MOVE
  {0x00,0x15,0x83},  // Bluetooth SIG / generic HC module
};
static const uint8_t SKIMMER_OUI_COUNT = 4;

// Known skimmer SSID name patterns
...
// OpenDroneID: WiFi NAN action frame, OUI 0x50-6F-9A, subtype 0x0D
struct DroneContact {
  char    id[21];        // UAV ID string
  float   lat, lon, alt; // operator/drone location
  float   speed;         // m/s
  uint8_t mac[6];
  int8_t  rssi;
  uint32_t lastSeen;
};
static DroneContact _drones[4];
static uint8_t      _droneCount = 0;

// ── Tracker/AirTag detection ─────────────────────────────────
// Apple AirTag, Tile, Samsung SmartTag WiFi probe OUIs
...
static TrackerContact _tags[8];
static uint8_t        _tagCount = 0;

// Known tracker OUI prefixes
static const uint8_t TRACKER_OUIS[][3] = {
  {0xF0,0x98,0x9D},  // Apple (AirTag, AirPods probe)
  {0x94,0x10,0x3E},  // Apple
  {0xDC,0x56,0xE7},  // Tile Inc
  {0x3C,0xBD,0xD8},  // Samsung (SmartTag)
  {0xFC,0x00,0x12},  // Samsung
  {0x88,0x36,0x6C},  // Chipolo
  {0x00,0x17,0xC4},  // Pebblebee
};
...
static SuspectAP _suspects[6];
static uint8_t   _suspectCount = 0;

// Track previous scan's RSSI for sudden appearance detection
struct RssiSnapshot { uint8_t bssid[6]; int8_t rssi; };
static RssiSnapshot _prevScan[8];
static uint8_t      _prevCount = 0;

// ── Promiscuous callback for drone NAN frames ────────────────
static void IRAM_ATTR _radarCallback(wifi_promiscuous_pkt_t* pkt, wifi_promiscuous_pkt_type_t type) {
  if (type != WIFI_PKT_MGMT) return;
  uint16_t len = pkt->rx_ctrl.sig_len; if (len>4) len-=4;
  if (len < 24) return;
  const uint8_t* p = pkt->payload;

  // Pwnagotchi: beacon frame (0x80) from DE:AD:BE:EF:DE:AD
  if (p[0] == 0x80 && memcmp(p+10, PWNA_MAC, 6)==0) {
    if (_hostileCount < 8) {
      bool known = false;
      for (uint8_t i=0; i<_hostileCount; i++)
        if (memcmp(_hostiles[i].mac, PWNA_MAC, 6)==0) { _hostiles[i].rssi=pkt->rx_ctrl.rssi; _hostiles[i].lastSeen=millis(); _hostiles[i].seenCount++; known=true; break; }
      if (!known) {
        HostileContact& h = _hostiles[_hostileCount++];
        memcpy(h.mac, PWNA_MAC, 6);
        strncpy(h.type, "PWNAGOTCHI", 15);
        // Try to extract name from beacon SSID field (offset 36)
        if (len > 38 && p[36]==0x00) {
          uint8_t slen = p[37]; if(slen>0&&slen<32&&36+2+slen<len) { memcpy(h.detail,p+38,slen); h.detail[slen]=0; }
          else strncpy(h.detail,"(no name)",47);
        } else strncpy(h.detail,"(no name)",47);
        h.rssi=pkt->rx_ctrl.rssi; h.firstSeen=h.lastSeen=millis(); h.seenCount=1;
  ...
  if (p[0] != 0xD0) return;
  // Check for NAN OUI: 50:6F:9A at vendor IE position
  // Action frame body starts at byte 24
  if (len < 32) return;
  const uint8_t* body = p + 24;
  // Category=4 (Public), Action=9 (Vendor Specific), OUI check
  ...
  // Parse basic ID message type 0x00 if present
  // For now just record the source MAC and RSSI
  // Full ASTM F3411 parsing would need 200+ bytes
  if (_droneCount < 4) {
...
// ── Scan for trackers and stingray suspects ──────────────────
static void _scanThreats() {
  NetworkRecon::enterCritical();
  const auto& nets = NetworkRecon::getNetworks();

  // Build current RSSI snapshot
  uint8_t snapCount = 0;
  RssiSnapshot snap[8];
  for (const auto& n : nets) {
  ...
  // Skimmer check — HC module OUI + SSID pattern on network list
  NetworkRecon::enterCritical();
  ...
  // Flipper Zero WiFi devboard — OUI + SSID pattern check
  for (const auto& n : nets2) {
...
void start() {
  ...
  if (!NetworkRecon::isRunning()) NetworkRecon::start();
  // Enable MGMT frames for drone NAN detection
  wifi_promiscuous_filter_t filt = {};
  filt.filter_mask = WIFI_PROMIS_FILTER_MASK_MGMT | WIFI_PROMIS_FILTER_MASK_DATA;
  esp_wifi_set_promiscuous_filter(&filt);
...
void update() {
  ...
  if (now-lm > 5000) {
    char buf[32];
    snprintf(buf,sizeof(buf),"D:%u T:%u S:%u K:%u H:%u", _droneCount,_tagCount,_suspectCount,_skimmerCount,_hostileCount);
    Mood::setStatusMessage(buf); lm=now;
  }
}
...
bool    isRunning()      { return _running; }
uint8_t getTab()         { return _tab; }
void    setTab(uint8_t t){ _tab = t % 5; }  // 5 tabs
uint8_t getDroneCount()  { return _droneCount; }
uint8_t getTagCount()    { return _tagCount; }
uint8_t getSuspectCount(){ return _suspectCount; }
uint8_t getSkimmerCount(){ return _skimmerCount; }
uint8_t getHostileCount(){ return _hostileCount; }
void getHostileInfo(uint8_t i, uint8_t* mac, char* type, char* detail, int8_t* rssi) {
...
void enterMode(PorkchopMode m) {
  ...
  switch(currentMode){
    ...
    case PorkchopMode::SNOUT_MODE:      SnoutMode::stop(); break;
    case PorkchopMode::SWINE_RADAR:     SwineRadar::stop(); break;
  ...
  switch(m){
    ...
    case PorkchopMode::SNOUT_MODE:      SnoutMode::start(); break;
    case PorkchopMode::SWINE_RADAR:     SwineRadar::start(); break;
...
// ============================================================
void handleInput() {
  // Ignore touch for first 3 seconds after boot to prevent spurious presses
  static uint32_t touchUnlockMs = 0;
  if (touchUnlockMs == 0) touchUnlockMs = bootTime + 5000;
  if (millis() < touchUnlockMs) return;
  ...
  switch (currentMode) {
      ...
      if (isTap) {
        ...
        if (zone == 0) {
          ...
          switch(menuSel) {
            ...
            case  6: enterMode(PorkchopMode::SNOUT_MODE); break;
            case  7: enterMode(PorkchopMode::SWINE_RADAR); break;
            case  8: enterMode(PorkchopMode::SPECTRUM_MODE); currentMode=PorkchopMode::SPECTRUM_MODE; break;
            case  9: currentMode=PorkchopMode::SETTINGS; break;
            case 10: currentMode=PorkchopMode::ABOUT; break;
            case 11: currentMode=PorkchopMode::CAPTURES; break;
            case 12: currentMode=PorkchopMode::SWINE_STATS; break;
            case 13: currentMode=PorkchopMode::BOUNTY_STATUS; break;
            case 14: enterMode(PorkchopMode::BOAR_BROS); break;
            case 15: currentMode=PorkchopMode::UNLOCKABLES; break;
            case 16: currentMode=PorkchopMode::DIAGNOSTICS; HeapHealth::setKnuthEnabled(true); break;
            case 17: currentMode=PorkchopMode::WEBUI_MODE; break;
    ...
    case PorkchopMode::SETTINGS:
      if(isTap){
        ...
        if (relY < MAIN_H/3) {
          ...
          char gmsg[32];
          if (cfg.ghostMode)
            snprintf(gmsg,sizeof(gmsg),"GHOST ON (%umin)", cfg.ghostInterval);
          else
    ...
    case PorkchopMode::SNOUT_MODE: {
      if (isTap) SnoutMode::setTab(SnoutMode::getTab()+1);
      if (isHold) { enterMode(PorkchopMode::IDLE); Display::showToast("SNOUT OFF",1200); }
      break;
    }

    case PorkchopMode::SWINE_RADAR: {
      if (isTap) SwineRadar::setTab(SwineRadar::getTab()+1);
      if (isHold) { enterMode(PorkchopMode::IDLE); Display::showToast("RADAR OFF",1200); }
      break;
    }
...
static void _sdSaveHS(const CapturedHandshake& hs) {
  ...
  WarTales::logCapture("HS", hs.ssid);
}
...
static void _sdSavePMKID(const CapturedPMKID& pm) {
  ...
  WarTales::logCapture("PMKID", pm.ssid);
}
...
static void _sendDiscover() {
  ...
  static uint32_t _lastDiscoverLog=0;
  if(millis()-_lastDiscoverLog>2000){ _lastDiscoverLog=millis();
    Serial.printf("[PIGSYNC] broadcasting CMD_DISCOVER ch=%d mac=%02X:%02X:%02X:%02X:%02X:%02X\n",
      PIGSYNC_DISCOVERY_CHANNEL,ownMac[0],ownMac[1],ownMac[2],ownMac[3],ownMac[4],ownMac[5]); }
...
void init() {
  ...
  // ESP-NOW broadcast RX requires AP or AP_STA mode — pure STA drops broadcast frames
  WiFi.mode(WIFI_AP_STA); WiFi.disconnect(); delay(50);
...
void start() {
  if(_running) return;
  NetworkRecon::pause();
  WiFi.mode(WIFI_AP_STA); WiFi.disconnect(false,true); delay(100);
  init();
...
void setup() {
  ...
  // LEDs — not present on NM-CYD-C5, skip
  // (LED_R/G/B_PIN are -1)
  ...
  // SD must be initialized BEFORE touch GPIO setup
  Serial.printf("[SD] Pre-init heap: %u largest: %u\n", ESP.getFreeHeap(), heap_caps_get_largest_free_block(MALLOC_CAP_8BIT));
  // On shared SPI bus: hold all CS pins HIGH before SD init
  // to prevent display/touch interfering with SD negotiation
  pinMode(SD_CS_PIN, OUTPUT);    digitalWrite(SD_CS_PIN, HIGH);
  pinMode(TOUCH_CS_PIN, OUTPUT); digitalWrite(TOUCH_CS_PIN, HIGH);
  pinMode(23, OUTPUT);           digitalWrite(23, HIGH);  // TFT CS
  delay(20);
  sdSPI.begin(6, 2, 7, SD_CS_PIN);  // CLK=6, MISO=2, MOSI=7, CS=10 (shared bus)
  for (int attempt = 0; attempt < 3 && !sdAvailable; attempt++) {
    Serial.printf("[SD]   attempt %d @ 400kHz...\n", attempt+1);
    if (SD.begin(SD_CS_PIN, sdSPI, 400000)) {
      sdAvailable = true;
      uint8_t ct = SD.cardType();
      Serial.printf("[SD] OK on CS=10 type=%s %lluMB\n",
  ...
  if (!sdAvailable) {
    ...
    if (!SD.exists("/captures")) { bool r=SD.mkdir("/captures"); Serial.printf("[SD] mkdir /captures: %d\n",r); }
    if (!SD.exists("/wigle"))    { bool r=SD.mkdir("/wigle");    Serial.printf("[SD] mkdir /wigle: %d\n",r); }
    if (!SD.exists("/logs"))     { bool r=SD.mkdir("/logs");     Serial.printf("[SD] mkdir /logs: %d exists:%d\n",r,(int)SD.exists("/logs")); }
  }
  ...
  // Touch — dedicated SPIClass on SPI2_HOST, CS-managed to avoid TFT_eSPI conflicts
  Touch::_touchSpiInit();
  ...
  NetworkRecon::setNewNetworkCallback([](wifi_auth_mode_t auth, bool hidden, const char* ssid, int8_t rssi, uint8_t ch){
    ...
    // Log every 10th new network to avoid flooding the tale log
    if (networkCount % 10 == 0) {
      char buf[48]; snprintf(buf,sizeof(buf),"net #%u: %.20s ch%d %ddBm", networkCount, hidden?"<hidden>":ssid, ch, rssi);
      WarTales::logEvent(buf);
    }
  });
  ...
  // Apply ghost mode if it was saved on
  if (cfg.ghostMode) {
    GhostMode::apply();
    Serial.println("[GHOST] ghost mode active from saved config");
  }
  WarTales::sessionStart();

  // Force first frame so screen is not black after boot splash
  tft.fillScreen(colorBG());
  Display::update();
}
...
void loop() {
  ...
  switch(currentMode){
    ...
    case PorkchopMode::SNOUT_MODE:      SnoutMode::update(); break;
    case PorkchopMode::SWINE_RADAR:     SwineRadar::update(); break;
